...
> **Docker users: no manual build required.** `docker build` (via `docker-compose.jetson.yml`)
> compiles the C++ core engine automatically inside the container. Skip this section unless
> you are developing the C++ pipeline directly on the device (outside Docker).
...
```bash
export NGC_API_KEY=<your_key>     # free account at ngc.nvidia.com
cd services/deepstream-jetson
./scripts/download_peoplenet.sh   # → models/peoplenet/resnet34_peoplenet_int8.onnx
./scripts/download_reid.sh        # → models/reid/resnet50_market1501_aicity156.onnx (deployable_v1.2)
```

Models go in `services/deepstream-jetson/models/`. The `docker-compose.jetson.yml` bind-mounts
this directory into the container at `/workspace/core_engine/models/`. TRT engine files are
auto-generated by `nvinfer` on first container start (~3–5 min on Jetson Orin NX) and written
back to the same host directory, so subsequent starts are fast.
...
| 5 | FastAPI control plane — `api/` (pipeline_manager, camera_registry, detection_store, routes) | **complete** |
| 6 | Production readiness — Dockerfile (C++ + Python in one image), start.sh, docker-compose.jetson.yml, structured logging | **complete** |
